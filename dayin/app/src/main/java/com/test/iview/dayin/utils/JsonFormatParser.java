...
public class JsonFormatParser implements JsonDeserializer<BaseResponse> {
    ...
    public BaseResponse deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        String mJson = mJsonObject.get("content").toString();
        // 判断是Array还是Object类型。
        if (mJsonObject.get("content").isJsonArray() && !mJsonObject.get("content").isJsonNull()) {
            mResult.setContent(fromJsonArray(mJson, mClass));
            mResult.setDataType(1);
        } else if (mJsonObject.get("content").isJsonObject() && !mJsonObject.get("content").isJsonNull()) {
            mResult.setContent(fromJsonObject(mJson, mClass));
            mResult.setDataType(0);
        } else if (mJsonObject.get("content").isJsonPrimitive() && !mJsonObject.get("content").isJsonNull()) {
            ...
            if (mNewJson.startsWith("[") || mNewJson.endsWith("]")) {
                mResult.setContent(fromJsonArray(mNewJson, mClass));
                mResult.setDataType(1);
            } else if (mNewJson.startsWith("{") || mNewJson.endsWith("}")) {
                mResult.setContent(fromJsonObject(mNewJson, mClass));
                mResult.setDataType(0);
            } else {
                mResult.setContent(fromJsonObject(mResult.toString(), mClass));
                mResult.setDataType(2);
            }
        } else if (mJsonObject.get("content").isJsonNull() || mJsonObject.get("content").getAsString().isEmpty()) {
//            mResult.setContent(fromJsonObject(mResult.toString(), mClass));
            mResult.setContent(null);
            mResult.setDataType(2);
        }
        // 根据键获取返回的状态码。
