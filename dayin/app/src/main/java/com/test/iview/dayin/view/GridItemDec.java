...
public class GridItemDec  extends RecyclerView.ItemDecoration {
    ...
    private GridItemDec(int horizonSpan,int verticalSpan,int color,boolean showLastLine) {
...
//            //满足条件( 最后一行 && 不绘制 ) 将vertical多出的一部分去掉;
    ...
    public static class Builder {
         ...
         * 是否最后一条显示分割线
         * */
        public GridItemDec.Builder setShowLastLine(boolean show){
            mShowLastLine = show;
            return this;
        }

        public GridItemDec build() {
            return new GridItemDec(mHorizonSpan, mVerticalSpan, mColor,mShowLastLine);
        }
    }
}